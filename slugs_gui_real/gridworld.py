...
class Gridworld():
    ...
    def __init__(self, filename=None, initial=0, nrows=8, ncols=8, nagents=1, targets=[], obstacles=[], moveobstacles = [], regions=dict()):
        ...
        if filename[0] != None:
            ...
            regions['deterministic'] = list(range(nrows * ncols))
        ...
        self.states = list(range(nrows*ncols))
    ...
    def coords(self, s):
        return (int(s / self.ncols), s % self.ncols)  # the coordinate for state s.

    def isAllowed(self, xxx_todo_changeme):
        (row,col) = xxx_todo_changeme
        if col not in list(range(self.ncols)) or row not in list(range(self.nrows)):
            return False
        return True

    def isAllowedState(self, xxx_todo_changeme1,returnState):
    ...
    def getProbRegions(self):
        probOfSuccess = dict([])
        for ground in list(self.regions.keys()):
            for direction in ['N', 'S', 'E', 'W']:
                if ground == 'pavement':
                    mass = random.choice(list(range(90, 95)))
                    massleft = 100 - mass
                    oneleft = random.choice(list(range(1, massleft)))
                    twoleft = massleft - oneleft
                if ground == 'gravel':
                    mass = random.choice(list(range(80, 85)))
                    massleft = 100 - mass
                    oneleft = random.choice(list(range(1, massleft)))
                    twoleft = massleft - oneleft
                if ground == 'grass':
                    mass = random.choice(list(range(85, 90)))
                    massleft = 100 - mass
                    oneleft = random.choice(list(range(1, massleft)))
                    twoleft = massleft - oneleft
                if ground == 'sand':
                    mass = random.choice(list(range(65, 70)))
                    massleft = 100 - mass
                    oneleft = random.choice(list(range(1, massleft)))
                    twoleft = massleft - oneleft
    ...
    def build_templates(self):
        ...
        template = int(self.size / 3) * template  # scale template
    ...
    def indx2coord(self, s, center=False):
        ...
        i, j = self.coords(s)
        if center:
            return i * (self.size + 1) + 1 + int(self.size / 2), \
                   j * (self.size + 1) + 1 + int(self.size / 2)
        else:
    ...
    def coord2indx(self, xxx_todo_changeme2):
        (x, y) = xxx_todo_changeme2
        return self.rcoords((int(x / (self.size + 1)), int(y / (self.size + 1))))
    ...
    def state2circle(self, state, bg=True, blit=True):
        ...
        for n in range(self.nagents):
            ...
            pygame.draw.circle(self.surface, (0+(50*n), 0+(20*n), 255.0/(n+1)), (y, x), int(self.size / 2))
        ...
        if self.bg_rendered:
            ...
            for n in range(self.nagents):
                ...
                for t in self.targets[n]:
                    ...
                    coords = pygame.Rect(y - int(self.size / 2), x - int(self.size / 2), self.size, self.size)
