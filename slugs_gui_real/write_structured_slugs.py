...
def parseJson(filename):
    ...
    for var in data['variables']:
            v = var.split('@')[0]
            if v not in list(variables.keys()):
...
def target_visibility(gw, invisibilityset, partitionGrid, target_vision_dist, filename_target_vis, allowed_states, visset_target):
    ...
    beliefcombs = powerset(list(partitionGrid.keys()))
        ...
        if (st in allowed_states) or (st == allstates[-1]):
            ...
            for s in tqdm(allowed_states):
                ...
                for iState in tqdm(beliefstates_invis):
                    for key,value in list(visset_target[iState].items()):
                        if not bool(value):
                            continue
                        for item_2 in value:
...
def write_to_slugs_part_dist(infile,gw,init,initmovetarget,invisibilityset,visset_target,targets,vel=1,visdist = 5,allowed_states = [],
    ...
    nonbeliefstates = gw.states
    beliefcombs = powerset(list(partitionGrid.keys()))
    #beliefcombs = powerset(list(range(len(partitionGrid))))
    allstates = copy.deepcopy(nonbeliefstates)
    for i in range(gw.nstates,gw.nstates + len(beliefcombs)):
        allstates.append(i)
    allstates.append(len(allstates)) # nominal state if target leaves allowed region
                        ...
                        if t in allowed_states and t not in repeat:
                            if t not in invisibilityset[s]:
                                ...
                                if not t == s and t not in targets: # not allowed to move on agent's position
                                    try:
                                        partgridkeyind = [inv for inv in range(len(list(partitionGrid.values()))) if t in list(partitionGrid.values())[inv]][0]
                                        t2 = list(partitionGrid.keys())[partgridkeyind]
                                        #t2 = list(range(len(partitionGrid[partgridkeyind])))
                                        beliefset.add(t2)
                                    except:
                                        print(t)
                            ...
                            for t in np.nonzero(gw.prob[gw.actlist[a]][b])[0]:
                                if t not in invisibilityset[s]:
                                    ...
                                    if t in allowed_states:
                                        t2 = list(partitionGrid.keys())[[inv for inv in range(len(list(partitionGrid.values()))) if t in list(partitionGrid.values())[inv]][0]]
                                        beliefset.add(t2)
                    if len(beliefset) > 0:
                        b2 = allstates[len(nonbeliefstates) + beliefcombs.index(beliefset)]
                        if b2 not in repeat:
    ...
    for s in tqdm(nonbeliefstates):
        if s in allowed_states:
            repeat = set()
            uset = list(itertools.product(list(range(len(gw.actlist))),repeat=vel))
            stri = "s = {} -> ".format(s)
            for u in uset:
...
#################################### -- Orientation stuff -- ##############################################
    if target_has_vision:
        ...
        for st,dict_of_states in list(target_vision_dict.items()):
            for s,vis_list in list(dict_of_states.items()):
        ...
        for state_key,state_value in list(visset_target.items()):
            if isinstance(state_value, set): # Check to see if the state_value is of type "set" instead of type "dict". "set" means that state_key corresponds to an obstacle.
                continue
            for state_card,state_vis_set in list(state_value.items()):
...
def write_to_slugs_imperfect_sensor(infile,gw,init,initmovetarget,invisibilityset,targets,vel=1, visdist=5,allowed_states = [],
    ...
    allstates = copy.deepcopy(list(sensor_uncertain_dict.keys()))
    ...
    file.write('[ENV_INIT]\n')
    print(sensor_uncertain_dict.values())
    print(sensor_uncertain_dict)
    print(initmovetarget)
    print([i for i in sensor_uncertain_dict.values() if initmovetarget in i])
    initbelief = list(sensor_uncertain_dict.values()).index(
                ...
                for st in sensor_uncertain_dict[belief_st]:
                    for a in range(gw.nactions):
                        for t in np.nonzero(gw.prob[gw.actlist[a]][st])[0]:
                            if t in allowed_states:
                                if t not in invisibilityset[s]:
                                    all_belief_gridstate_sets = [i for i in list(sensor_uncertain_dict.values()) if t in i]
                                    for belief_gridstates in all_belief_gridstate_sets:
                                        next_belief_gridstate = list(sensor_uncertain_dict.keys())[list(sensor_uncertain_dict.values()).index(belief_gridstates)]
                                    ...
                                    if not t == s and t not in targets:  # not allowed to move on agent's position
                                        try:
                                            partgridkeyind = [inv for inv in range(len(list(partitionGrid.values()))) if
                                                              t in list(partitionGrid.values())[inv]][0]
                                            t2 = list(partitionGrid.keys())[partgridkeyind]
                                            beliefset.add(t2)
                                        except:
                                            print(t)
                            ...
                            for t in np.nonzero(gw.prob[gw.actlist[a]][b])[0]:
                                if t in allowed_states:
                                    if t not in invisibilityset[s]:
                                        all_belief_gridstate_sets = [i for i in list(sensor_uncertain_dict.values()) if t in i]
                                        for belief_gridstates in all_belief_gridstate_sets:
                                            next_belief_gridstate = list(sensor_uncertain_dict.keys())[
                                                list(sensor_uncertain_dict.values()).index(belief_gridstates)]
                                            if next_belief_gridstate not in repeat:
                                                stri += 'st\' = {} \\/'.format(next_belief_gridstate)
                                                repeat.add(next_belief_gridstate)
                                    else:
    ...
    for s in allowed_states:
        ...
        for t in borderstates:
            if not t in invisstates:
                ...
                t2 = list(partitionGrid.keys())[
                    [inv for inv in range(len(list(partitionGrid.values()))) if t in list(partitionGrid.values())[inv]][
                        0]]
                beliefset.add(t2)
        if len(beliefset) > 0:
            b2 = allstates[belief_nrows * belief_ncols + beliefcombs.index(beliefset)]
    ...
    if belief_liveness >= sensor_uncertainty:
        for st in range(belief_nrows*belief_ncols):
            stri += 'st = {}'.format(st)
            if st != list(range(belief_nrows*belief_ncols))[-1]:
                stri += ' \\/ '
        for b in beliefcombs:
            beliefset = set()
            for beliefstate in b:
